...
namespace CAE
{
    ...
    {
        // employing two - point Gaussian integralȨ��Ϊ1
        ...
        GP2.resize(n_interF * n_gps, 3); // ��ʼ�������������ֵ�����*���紦�����嵥Ԫ����
        int t = 0;                       // ��¼���紦ϸ�������ϸ�˹���ܸ���
        // �����浥Ԫѭ��
        for (int e = 0; e < n_interF; e++) // �����洦ϸ�������
        {
            MatrixXd nodes1(4, 3);                             // �ı��ν�����ڵ���
            MatrixXd pts1(8, 3), pts2(8, 3);                   // ���紦�֡�ϸ�����嵥Ԫ�ڵ�����
            vector<int> sctr1(8), sctr2(8);                    // ���紦�֡�ϸ�����嵥Ԫ�ڵ���
            int b_node1 = data_cae.bndFace_finemesh[e][0] - 1; // �ڵ�������
    ...
    {
        const auto &node_topos_BndMesh_F = data_cae.node_topos_[data_cae.BndMesh_F[e] - 1]; // ����Ҫ��1
    ...
    */
                                        ...
                                        vector<double> &nz_val, vector<int> &row_idx, vector<int> &col_idx)
    {
        ...
        for (int e = 0; e < nF_bmesh; e++) // �����洦ϸ�������
        {
            MatrixXd pts1(8, 3), pts2(8, 3); // ���紦ϸ���������嵥Ԫ�ڵ�����
            vector<int> sctr1(8), sctr2(8);  // ���紦ϸ���������嵥Ԫ�ڵ���
            GetIntF_ele_Inform(data_cae, sctr1, sctr2, pts1, pts2, e);
            ...
            for (int q = 4 * e; q < 4 * e + 4; q++)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    B1(3, 3 * i) = T_dN1dx(1, i);
                    B1(3, 3 * i + 1) = T_dN1dx(0, i);
                    B1(4, 3 * i + 1) = T_dN1dx(2, i);
                    B1(4, 3 * i + 2) = T_dN1dx(1, i);
                    B1(5, 3 * i) = T_dN1dx(2, i);
                    B1(5, 3 * i + 2) = T_dN1dx(0, i);

                    B2(0, 3 * i) = T_dN2dx(0, i);
                    B2(1, 3 * i + 1) = T_dN2dx(1, i);
                    B2(2, 3 * i + 2) = T_dN2dx(2, i);
                    B2(3, 3 * i) = T_dN2dx(1, i);
                    B2(3, 3 * i + 1) = T_dN2dx(0, i);
                    B2(4, 3 * i + 1) = T_dN2dx(2, i);
                    B2(4, 3 * i + 2) = T_dN2dx(1, i);
                    B2(5, 3 * i) = T_dN2dx(2, i);
                    B2(5, 3 * i + 2) = T_dN2dx(0, i);
    ...
    double NCF_map::Get_Alpha(data_management &data_cae, elastic_mat &data_mat, MatrixXd &pts1)
    {
        ...
        y2 = pts1(1, 1);
        z2 = pts1(1, 2);
        double h = sqrt(pow(x1 - x2, 2) + pow(y1 - y2, 2) + pow(z1 - z2, 2)); // ��Ԫ�ߴ��С
                                         ...
                                         data_management &data_cae, int &e)
    {
        ...
        { /*
           ...
           */
            ...
            int ncf_F = data_cae.BndMesh_F[e] - 1; // ϸ����Ԫ��Ŵ�0��ʼ
            ...
            int ncf_C = data_cae.BndMesh_C[e] - 1; // ������Ԫ��Ŵ�0��ʼ
