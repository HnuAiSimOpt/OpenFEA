...
namespace CAE
{
    ...
    void assamble_stiffness::fill_CSR_sparse_mat(data_management &data_cae, elastic_mat &data_mat)
    {
        ...
        cout << "The CSR has been filled" << endl;
    }
    ...
    //[********拓扑优化部分******]
    void assamble_stiffness::topo_fill_CSR_sparse_mat(data_management &data_cae, elastic_mat &data_mat, vector<double> &pseudo_density)
    {
        // 声明内存
        this->nz_val_.clear();
        this->nz_val_.resize(num_nz_val_);
        std::fill(this->nz_val_.begin(), this->nz_val_.end(), 0.);
        // 初始化单元
        data_cae.ele_inite(data_mat);
        // 初始化单元坐标，刚度矩阵
        int num_nodes;
                ...
                if (jj_dof >= 0)
                {
                    ...
                    for (int nn = 0; nn < loop_size; nn++)
                    {
                        ...
                        if (ii_dof >= 0)
                        {
                            ...
                            nz_val_[t] = nz_val_[t] + pseudo_density[id_ele] * stiffness_matrix(mm, nn);
                        }
                    }
                }
            }
        }
        cout << "the CSR of Topo has been filled, ";
    };
