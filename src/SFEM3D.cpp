...
namespace CAE
{
    void SFEM3D::build()
    {
        ...
        for (int i = 0; i < data_cae->ne_; i++) {
            ...
            e->n = i + 1;
            e->sfemData = this;
            this->SmoothDomainList.push_back(e);
        }
    }
    ...
    void SFEM3D::eleData()
    {
        ...
        for(int i = 0; i < data_cae->ne_;i++){
            ...
            const vector<double>& p1 = data_cae->coords_[n1 - 1];
            const vector<double>& p2 = data_cae->coords_[n2 - 1];
            const vector<double>& p3 = data_cae->coords_[n3 - 1];
            const vector<double>& p4 = data_cae->coords_[n4 - 1];
